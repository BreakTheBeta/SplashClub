  ...
  useEffect(() => {
    if (lastMessage !== null) {
      try {
        ...
        } else if (data.type === 'all_answers_submitted' || data.type === 'vote_start') { 
        ...
        console.error("Failed to parse WebSocket message in Prompt:", e, lastMessage.data);
  ...
  return (
    <div className={`container mx-auto px-4 py-8 ${theme.background.page}`}>
      <div className={`border ${theme.border} rounded-lg p-6 shadow-sm ${theme.background.card} max-w-lg mx-auto`}>
        ...
        <div className={`my-6 px-4 py-3 border-l-4 ${theme.border} ${theme.background.highlight || ''} rounded-r-md`}>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <Input
            ...
            isValid={answer.trim().length === 0 ? null : answer.trim().length > 0}
          />
          <Button
            type="submit"
